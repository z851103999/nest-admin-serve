...
export class LogService {
  ...
  async operLogList(
    ...
    if (reqOperLogDto.params) {
      where.operTime = Between(
        ...
        dayjs(reqOperLogDto.params.endTime).add(1, 'day').format(),
      );
    }
  ...
  async addLogininfor(request: Request, msg: string, token?: string) {
    ...
    logininfor.msg = msg;
    logininfor.loginTime = dayjs().format('YYYY-MM-DDTHH:mm:ss');
  ...
  async logininforList(
    ...
    if (reqLogininforDto.params) {
      where.loginTime = Between(
        ...
        dayjs(reqLogininforDto.params.endTime).add(1, 'day').format(),
      );
    }
